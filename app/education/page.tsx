    ...
    <FadeIn className="space-y-8 rounded-[28px] border border-white/10 bg-slate-950/75 p-6 md:p-8">
      ...
      <div className="space-y-5">
        {educationTimeline.map((item) => (
          <article key={`${item.title}-${item.period}`} className="rounded-2xl border border-white/10 bg-slate-900/70 p-5">
      ...
      <div className="rounded-2xl border border-white/10 bg-slate-900/70 p-5">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          {languages.map(({ language, level }) => (
            <div key={language} className="rounded-xl border border-white/10 bg-white/3 p-3">
